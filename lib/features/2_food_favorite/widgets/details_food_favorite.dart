...
class DetailsFoodFavorite extends StatelessWidget {
  final FoodModel food;

  const DetailsFoodFavorite({required this.food, Key? key}) : super(key: key);
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildNutrientGroup(
              ...
              food.foodNutrients
                      ?.where(
                          (nutrient) => _isVitamin(nutrient.nutrient?.number))
                      .toList() ??
                  []),
          const SizedBox(height: 20),
          _buildNutrientGroup(
              ...
              food.foodNutrients
                      ?.where(
                          (nutrient) => _isMineral(nutrient.nutrient?.number))
                      .toList() ??
                  []),
          const SizedBox(height: 20),
          _buildNutrientGroup(
  ...
  Widget _buildNutrientGroup(
      String groupName, List<FoodNutrientsModel>? nutrients) {
    return Card(
      elevation: 5,
      child: Padding(
